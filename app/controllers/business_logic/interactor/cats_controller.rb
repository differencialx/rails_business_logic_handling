  ...
  module Interactor
    class CatsController < ApplicationController
      ...
      cases do
        ...
        match(:bad_request) { |result| result.failure? && result.semantic_status == 400 }
        match(:not_found) { |result| result.failure? && result.semantic_status == 404 }
        match(:unprocessable_entity) { |result| result.failure? && result.semantic_status == 422 }
        match(:server_error) { |result| result.failure? && result.semantic_status == 500 }
        match(:service_unavailable) { |result| result.failure? && result.semantic_status == 503 }
      end
      ...
      def index
        endpoint operation: Interactors::Cats::Index::Organizer,
      ...
      def update
        endpoint operation: Interactors::Cats::Update::Organizer,
                 renderer_options: { serializer: Cats::CatSerializer },
                 options: {
