  ...
  module Yaso
    class CatsController < ApplicationController
      ...
      cases do
        ...
        match(:bad_request) { |result| result.failure? && result[:semantic] == 400 }
        match(:not_found) { |result| result.failure? && result[:semantic] == 404 }
        match(:unprocessable_entity) { |result| result.failure? && result[:semantic] == 422 }
        match(:server_error) { |result| result.failure? && result[:semantic] == 500 }
        match(:service_unavailable) { |result| result.failure? && result[:semantic] == 503 }
      end

      handler do
        on(:success) do |result, **opts|
          render json: {
            data: opts[:serializer].new(result[:model]).to_h, pagy: result[:pagy_meta] },
            status: 200
        end
      ...
      def index
        endpoint operation: YasoServices::Cats::Index,
      ...
      def update
        endpoint operation: YasoServices::Cats::Update,
                 renderer_options: { serializer: Cats::CatSerializer },
                 options: {
